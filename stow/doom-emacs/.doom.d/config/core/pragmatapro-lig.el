...
;; Version: 20231222 (MERGED with contents from pragmatapro-prettify-symbols-alist in pragmatapro-prettify-symbols.el)
    ...
    (let ((s start) (e end)
      ...
      (while (and (> s ss)
                  (aref pragmatapro-lig-use-table
                        (min 10239 (or (char-before s) 127))))
        (setq s (1- s)))
      (while (and (< e ee)
                  (aref pragmatapro-lig-use-table
                        (min 10239 (or (char-after e) 127))))
        (setq e (1+ e)))
      (cons s e))))
